...
type Item struct {
	...
	ItemTypeID          int `gorm:"index:unique_item_type_id"`
	ItemType            ItemType
	ItemData            datatypes.JSON
	Price               float64   `gorm:"index:unique_item_price"`
	DateTime            time.Time `gorm:"index:unique_item_date_time"`
	DeleteAfter         sql.NullTime
	DecreasePriceAfter  sql.NullTime
	DecreasePriceAmount float64
	Location            string `gorm:"index:unique_item_location"`
	TemporaryPassword   string
	Currency            string
	Public              bool
	Premium             bool
	PremiumDateTime     time.Time `gorm:"index:unique_item_premium_date_time"`
